...
# News
- v1.6.0 : security fix: `deserialize` rejects an oversized input
...
then, you can use the following [eth2.0 functions](https://github.com/ethereum/eth2.0-specs/blob/dev/specs/phase0/beacon-chain.md#bls-signatures).
...
Since v1.5.0, `src/bls_c.js` is built with clang (wasm32 target), `wasm-ld` (lld), `wasm-opt` (binaryen) and python3 instead of Emscripten.
```
git submodule update --init --recursive
make -C src
```
Use `make -C src LLVM_VER=-21` to select a versioned clang/wasm-ld such as `clang-21`.
`src/bls_c.js` and `browser/bls.js` in the repository are regenerated by the GitHub Action `build.yml` on the `dev` branch.
